...
在 DSH 网页侧栏底部（设置按钮上方）加一个「¥ 模型 / 余额」入口，点开是一个可拖拽面板（内容只针对**基元律动**）：

- **模型 / 余额** — 面板页签只有这两枚（分段式按钮）：模型页签顶部有**分类筛选**（全部 / 文本 / 图像 / 音频 / 视频 / 向量，带计数，口径与平台模型列表页一致），下方以卡片网格展示模型（显示名、上下文、输入/输出/缓存单价、折扣价、图片单价、能力徽标）；余额页签主卡展示**剩余限时额度**与到期时间，下方为**当日使用**（本地 0 点起聚合调用日志），每 60s 自动刷新。
- **设置（右上角 ⚙）** — 粘贴/清除基元律动网页会话 Cookie（`tr_session`），以及 API Key 的读取状态。
...
- `lib/client.js` — browser 半区：侧栏入口按钮 + 可拖拽三页签面板（React，经 `window.__ModuleLoader__` 加载）
