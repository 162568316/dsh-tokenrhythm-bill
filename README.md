...
- **模型 / 余额 / 密钥** — 面板页签共三枚（分段式按钮）：模型页签顶部有**分类筛选**（全部 / 文本 / 图像 / 音频 / 视频 / 向量，带计数，口径与平台模型列表页一致），下方以卡片网格展示模型（显示名、上下文、输入/输出/缓存单价、折扣价、图片单价、能力徽标）；余额页签主卡以**账户余额**为主数字，限时额度作副位并带**倒计时胶囊**（≤3 天转警示色）与**限时占比条**，下方为**当日使用**（本地 0 点起聚合调用日志）与**近 7 天花费柱状图**（悬停浮出当日各模型消费明细，Top 6 + 其他），每 60s 自动刷新；密钥页签可查看/新建平台 API Key。
