...
// ---- 桌面通知：episode 去重判定（纯函数） ----
// 阈值与 client 端 alertOf 同口径：低余额 < ¥10，临期 ≤ 3 天。
export const NOTIFY_LOW_BALANCE_CNY = 10
export const NOTIFY_EXPIRING_DAYS = 3

/**
 * 计算本轮该弹的系统通知与去重状态。DSH 原生 desktopRuntime.notifyAttention 通道。
 * - 低余额：availableBalanceCny < 10 且 prev.lowBalanceKey 为空 → 发一条并记 'low'；
 *   余额恢复 ≥10 → 清键（下次再跌破才重发）。
 * - 临期：逐笔 expiringItems 中 days ≤ 3 且 exp:<expireAt> 未记录 → 合并成一条通知，
 *   全部涉及 key 都记入 state（不重发）；keys 上限 20 条，列表中已过期的顺带清理。
 * - prefsNotify 为 false → 不发（prev 原样保留，重开后不重弹旧状态）。
 * - 单轮最多 2 条：低余额 1 条 + 临期 1 条。
 */
export function computeNotifyEpisode(balance, expiringItems, prefsNotify, prev, now) {
  const t = Number.isFinite(now) ? now : Date.now()
  const prevKeys = prev && isObj(prev) ? prev : {}
  const prevLow = typeof prevKeys.lowBalanceKey === 'string' ? prevKeys.lowBalanceKey : ''
  const prevExp = Array.isArray(prevKeys.expiringKeys) ? prevKeys.expiringKeys.filter((k) => typeof k === 'string') : []
  const notifications = []
  const notifiedExpKeys = []
  let lowBalanceKey = prevLow
  if (prefsNotify !== false) {
    const b = balance && isObj(balance) ? balance : {}
    const available = toNum(b.availableBalanceCny ?? b.available_balance_cny)
    if (available !== null) {
      if (available < NOTIFY_LOW_BALANCE_CNY) {
        if (lowBalanceKey === '') {
          lowBalanceKey = 'low'
          notifications.push({
            key: 'low',
            title: '基元律动 · 余额不足',
            body: '可用余额 ¥' + (Math.round(available * 100) / 100) + '，低于 ¥10，请及时充值',
          })
        }
      } else if (lowBalanceKey !== '') {
        lowBalanceKey = '' // 恢复 → 清键，下次再跌破才重发
      }
    }
    // 临期逐笔：按到期升序，≤3 天的都算涉及（合并为一条通知，全部 key 记档）
    const items = (Array.isArray(expiringItems) ? expiringItems : [])
      .map((it) => isObj(it) ? { amountCny: toNum(it.amountCny), expireMs: Date.parse(asStr(it.expireAt)) } : null)
      .filter((it) => it !== null && it.amountCny !== null && it.amountCny > 0 && Number.isFinite(it.expireMs))
      .sort((a, b) => a.expireMs - b.expireMs)
    const fresh = []
    for (const it of items) {
      if (it.expireMs <= t) continue // 已过期不再提醒
      const days = Math.max(0, Math.ceil((it.expireMs - t) / 86400000))
      if (days > NOTIFY_EXPIRING_DAYS) break // 升序，后面更远
      const key = 'exp:' + new Date(it.expireMs).toISOString()
      if (prevExp.includes(key)) continue
      fresh.push({ it, key, days })
    }
    if (fresh.length > 0) {
      const first = fresh[0]
      notifications.push({
        key: first.key,
        title: '基元律动 · 限时余额即将失效',
        body: '¥' + (Math.round(first.it.amountCny * 100) / 100) + ' 将在 ' + first.days + ' 天后失效'
          + (fresh.length > 1 ? '；另有 ' + (fresh.length - 1) + ' 笔临近' : ''),
      })
      for (const f of fresh) notifiedExpKeys.push(f.key)
    }
  }
  // 汇总 next：追加本轮涉及的全部临期 key；清理「列表内已过期」笔的 key
  // （列表里查不到的 key 保守保留，防上游抖动导致重发）；cap 20（尾追加截前头）。
  const expiryMsByKey = new Map()
  for (const it of (Array.isArray(expiringItems) ? expiringItems : [])) {
    if (isObj(it)) {
      const ms = Date.parse(asStr(it.expireAt))
      if (Number.isFinite(ms)) expiryMsByKey.set('exp:' + new Date(ms).toISOString(), ms)
    }
  }
  const nextExp = prevExp.slice()
  for (const key of notifiedExpKeys) {
    if (!nextExp.includes(key)) nextExp.push(key)
  }
  const cleaned = nextExp.filter((k) => {
    const ms = expiryMsByKey.get(k)
    return ms === undefined || ms > t
  })
  const expiringKeys = cleaned.length > 20 ? cleaned.slice(cleaned.length - 20) : cleaned
  return { notifications, next: { lowBalanceKey, expiringKeys } }
}
...
export function apply(ctx) {
  ...
  let state = { cookie: '', csrf: '', prefs: {}, accounts: [], activeAccount: '', update: {}, notify: { lowBalanceKey: '', expiringKeys: [] } }
  ...
  const loadState = () => {
    ...
    try {
      ...
      if (isObj(data)) {
        ...
        state.notify = sanitizeNotify(data.notify)
      }
    } catch { /* 不可读 → 空状态 */ }
    return state
  }
  const saveState = () => {
    ...
    try {
      ...
      writeFileSync(file, JSON.stringify({ cookie: state.cookie, csrf: state.csrf, prefs: state.prefs, accounts: state.accounts, activeAccount: state.activeAccount, update: state.update, notify: state.notify, savedAt: Date.now() }, null, 2) + '\n', { encoding: 'utf8', mode: 0o600 })
    } catch { /* best-effort：丢持久化不丢功能 */ }
  }

  // 桌面通知去重状态清洗：只留合法键（低余额 key 字符串 + 临期 key 字符串数组 ≤20）。
  const sanitizeNotify = (raw) => {
    const out = { lowBalanceKey: '', expiringKeys: [] }
    if (!isObj(raw)) return out
    if (typeof raw.lowBalanceKey === 'string') out.lowBalanceKey = raw.lowBalanceKey
    if (Array.isArray(raw.expiringKeys)) out.expiringKeys = raw.expiringKeys.filter((k) => typeof k === 'string').slice(0, 20)
    return out
  }
  ...
  const sanitizePrefs = (input) => {
    ...
    const nv = toBool(input.notify)
    if (nv !== null) out.notify = nv
    return out
  }

  // ---- 桌面通知：DSH 原生 desktopRuntime.notifyAttention（DSH 官方「任务完成」提醒
  // 同一条通道；仅 DSH Desktop 桌面壳提供此服务）。纯 web 启动无 desktopRuntime，
  // 这个动态注入的 fiber 永远挂起不执行——无通知、不 crash，其余功能不受影响。 ----
  let notifyFn = null
  ctx.inject(['desktopRuntime'], (desktopCtx) => {
    notifyFn = (n) => { try { desktopCtx.desktopRuntime.notifyAttention(n) } catch { /* 通知失败不阻塞轮询 */ } }
  })

  // 余额到手后判定并弹通知（同步、去重状态落盘）。由 /balance 路由驱动——
  // 客户端预警轮询（面板关着也 5 分钟一次）天然保证了调用频率。
  const maybeNotify = (balance) => {
    if (typeof notifyFn !== 'function') return
    loadState()
    const prefsNotify = toBool(state.prefs.notify) !== false // 默认开启
    const r = computeNotifyEpisode(
      balance,
      Array.isArray(balance && balance.expiringItems) ? balance.expiringItems : [],
      prefsNotify,
      state.notify,
    )
    if (r.notifications.length === 0 && r.next.lowBalanceKey === state.notify.lowBalanceKey
      && r.next.expiringKeys.length === state.notify.expiringKeys.length
      && r.next.expiringKeys.every((k, i) => k === state.notify.expiringKeys[i])) return
    state.notify = r.next
    saveState()
    for (const n of r.notifications) notifyFn(n)
  }
  ...
  const serve = async (req, res) => {
    try {
      ...
      if (pathname === '/dsh-tokenrhythm-bill/manifest' && req.method === 'GET') {
        ...
        // 数据账号标注：账号密码模式用 activeAccount；Cookie 模式没有它，
        // 用带缓存的 /api/me 取平台账户名（失败静默 → 前端回退 Cookie 模式文案）。
        let sessionAccount = state.activeAccount || null
        if (!sessionAccount && state.cookie !== '') sessionAccount = await fetchMeAccountName(state.cookie)
        writeJson(res, {
          ok: true,
          version: PKG_VERSION,
          ...
          session: { configured: state.cookie !== '', hint: maskSecret(state.cookie), account: sessionAccount || null },
          error,
        })
        return
      }
      ...
      if (pathname === '/dsh-tokenrhythm-bill/balance' && req.method === 'GET') {
        ...
        try {
          ...
          maybeNotify(balance) // 桌面通知判定（episode 去重，状态落盘）
      ...
      if (pathname === '/dsh-tokenrhythm-bill/session' && req.method === 'POST') {
        ...
        saveState()
        // 粘贴会话后立即回带账户名（Cookie 模式 activeAccount 为空，走 /api/me 取）。
        let sessionAccount = state.activeAccount || null
        if (!sessionAccount && cookie !== '') sessionAccount = await fetchMeAccountName(cookie)
        writeJson(res, { ok: true, configured: cookie !== '', hint: maskSecret(cookie), account: sessionAccount || null })
        return
      }
      ...
      const trMutate = async (path, init) => {
        ...
        return attempt(state.cookie, state.csrf)
      }
