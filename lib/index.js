...
export function apply(ctx) {
  ...
  // ---- 近 7 天花费趋势（本地日分桶，5 分钟缓存，避免每次余额刷新都翻页）----
  const TREND_TTL_MS = 5 * 60 * 1000
  const TREND_MAX_PAGES = 15
  let trendCache = null // { data, ts }
  const fetchUsageTrend = async (cookie) => {
    if (trendCache !== null && Date.now() - trendCache.ts < TREND_TTL_MS) return trendCache.data
    const days = 7
    const start = new Date()
    start.setDate(start.getDate() - (days - 1))
    start.setHours(0, 0, 0, 0)
    const end = new Date()
    // 按本地日预建桶（toDateString 即本地时区日期）。
    const buckets = new Map()
    for (let i = 0; i < days; i++) {
      const d = new Date(start)
      d.setDate(d.getDate() + i)
      buckets.set(d.toDateString(), { date: (d.getMonth() + 1) + '-' + d.getDate(), costCny: 0, calls: 0 })
    }
    const headers = trHeaders(cookie)
    for (let page = 1; page <= TREND_MAX_PAGES; page++) {
      ...
      for (const item of list) {
        ...
        if (b) { b.costCny += toNum(item.costCny) ?? 0; b.calls++ }
      }
      if (list.length < DAILY_LOG_PAGE_SIZE) break
    }
    const out = [...buckets.values()].map((b) => ({ date: b.date, costCny: Math.round(b.costCny * 1000) / 1000, calls: b.calls }))
    trendCache = { data: out, ts: Date.now() }
    return out
  }
