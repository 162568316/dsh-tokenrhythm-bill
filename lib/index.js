...
export function apply(ctx) {
  ...
  // ---- 花费趋势：最近 7 个有调用记录的日子（本地日分桶 + 按模型细分，5 分钟缓存）----
  ...
  const fetchUsageTrend = async (cookie) => {
    ...
    const end = new Date()
    // 按需建桶（有记录的日子才有桶；toDateString 即本地时区日期）；models 供柱状图悬停明细。
    ...
    const ingest = (list) => {
      for (const item of list) {
        ...
        if (t.getTime() < oldestSeen) oldestSeen = t.getTime()
        const dayStart = new Date(t)
        dayStart.setHours(0, 0, 0, 0)
        const key = dayStart.toDateString()
        let b = buckets.get(key)
        if (!b) {
          b = { key, t: dayStart.getTime(), date: (dayStart.getMonth() + 1) + '-' + dayStart.getDate(), costCny: 0, calls: 0, models: new Map() }
          buckets.set(key, b)
        }
    ...
    for (let p = 2; p <= TREND_MAX_PAGES && !complete(); p += TREND_BATCH) {
      ...
      const parts = await Promise.all(batch.map((pg) => fetchPage(pg)))
      let any = false
      for (const j of parts) {
        if (j === null) continue
        const d = unwrapEnvelope(j)
        if (isObj(d) && Array.isArray(d.list) && d.list.length > 0) { items.push(...d.list); ingest(d.list); any = true }
      }
      if (!any) break // 整批无数据：越界/接口异常，后续页不会再有新记录
    }
    // 只取最近 7 个有调用记录的日子（无记录日不留痕、不占柱位），按时间升序给图
