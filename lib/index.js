...
export function apply(ctx) {
  ...
  let state = { cookie: '', csrf: '', prefs: {}, accounts: [], activeAccount: '' }
  ...
  const saveState = () => {
    ...
    try {
      ...
      writeFileSync(file, JSON.stringify({ cookie: state.cookie, csrf: state.csrf, prefs: state.prefs, accounts: state.accounts, activeAccount: state.activeAccount, savedAt: Date.now() }, null, 2) + '\n', { encoding: 'utf8', mode: 0o600 })
