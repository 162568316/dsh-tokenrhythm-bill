  ...
  const serve = async (req, res) => {
    try {
      ...
      if (pathname === '/dsh-tokenrhythm-bill/stepfun/key-create' && req.method === 'POST') {
        ...
        console.log('[stepfun/key-create] body=', JSON.stringify(body), 'name=', JSON.stringify(name))
        if (name === '' || name.length > 20) { writeJson(res, { ok: false, error: '请填写密钥名称（平台要求，不超过 20 字符）', _debug: { parsed: JSON.stringify(body), name: JSON.stringify(name), nameLen: name.length } }, 400); return }
      ...
      if (pathname === '/dsh-tokenrhythm-bill/stepfun/key-delete' && req.method === 'POST') {
        const body = await readBody(req)
        const keyId = typeof body && body.keyId === 'string' ? body.keyId.trim() : (body && typeof body.keyId === 'number' ? String(body.keyId) : '')
        if (keyId === '') { writeJson(res, { ok: false, error: '缺少 keyId' }, 400); return }
