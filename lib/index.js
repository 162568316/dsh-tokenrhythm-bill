...
export function apply(ctx) {
  ...
  // 实测：pageSize 被服务端锁死 100（500 直接 400），但响应带 total。重用户 7 天可超数千次
  // 调用，旧版「15 页×100」会静默截断——最新优先排序让被截掉的恰好是较早的日子，图上
  // 呈现幽灵 ¥0。改为：第一页拿 total 精确定页数，余页 6 路并行批拉，总量硬上限封顶；
  // 仍装不下的如实标注 truncated（前端出提示），不再装准。
  ...
  const fetchUsageTrend = async (cookie) => {
    ...
    const headers = trHeaders(cookie)
    const pageQs = (page) => 'startAt=' + encodeURIComponent(start.toISOString()) + '&endAt=' + encodeURIComponent(end.toISOString())
      + '&page=' + page + '&pageSize=' + DAILY_LOG_PAGE_SIZE
    const fetchPage = async (page) => {
      try {
        const res = await fetchWithTimeout(TOKENRHYTHM_BASE + '/api/call-logs/page?' + pageQs(page), { headers })
    ...
    // total 精确定页数（无 total 时退化为拉满预算）；硬上限防极端大用户拖垮余额加载
    const total = isObj(firstData) && Number.isFinite(Number(firstData.total)) ? Number(firstData.total) : 0
    const maxPages = Math.max(1, Math.min(total > 0 ? Math.ceil(total / DAILY_LOG_PAGE_SIZE) : TREND_MAX_PAGES, TREND_MAX_PAGES))
    const items = [...firstList]
    const rest = []
    for (let p = 2; p <= maxPages; p++) rest.push(p)
    for (let i = 0; i < rest.length; i += TREND_BATCH) {
      const batch = rest.slice(i, i + TREND_BATCH)
      const parts = await Promise.all(batch.map((p) => fetchPage(p)))
      let exhausted = false
      for (const j of parts) {
        if (j === null) continue
        const d = unwrapEnvelope(j)
        if (isObj(d) && Array.isArray(d.list) && d.list.length > 0) items.push(...d.list)
        else exhausted = true // 空页/越界：后续页不会再有数据
      }
      if (exhausted || (total > 0 && items.length >= total)) break
    }
    ...
    trendCaches.set(cacheKey, { data: result, ts: Date.now() })
  ...
  const fetchUpstreamBalance = async (cookie, retried) => {
    ...
    const [daily, trendAll, recent] = await Promise.all([
    ...
    return {
      ...
      trendMeta: trendAll ? { truncated: !!trendAll.truncated, fetched: trendAll.fetched || 0, total: trendAll.total || 0 } : null,
      recent,
    }
  }
