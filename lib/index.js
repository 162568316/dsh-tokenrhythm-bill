...
// 最贴合的窗口，且汇总/按模型/按客户端/Top 花费都能一次拿到。
const USAGE_DAYS = 30
const USAGE_RANGE = '30d'
// 按天窗口单页条数：call-logs/page 与 usage/panel 的 pageSize 上限都是 100。
const USAGE_DAY_PAGE_SIZE = 100
// 每日模型明细行数上限（tooltip 里要能放下，尾部散着的小模型不进浮层）。
const MODEL_ROWS_PER_DAY = 6
// Top 花费调用条数（usage/panel 的 topCostCalls 本身就是 3 条，多要也拿不到）。
...
export function apply(ctx) {
  ...
  //      Top 花费调用。它不支持任何按天参数（实测 groupBy/startAt/endAt/dimension
  //      全被忽略，响应与不带时逐字节相同），所以日历的按天数据不能问它；而且它的
  //      游标翻页第 3 页起必然 503，想靠翻 items 拼出 30 天要约 99 个请求。
  //   2) /api/call-logs/page 按天绝对窗口 —— 一天一个窗口拿当日 total（精确调用数）
  //      与当页明细。
  //
  // 请求量控制：整个页签只需 **2 个请求**——panel 拿汇总/拆分/Top 花费，usage-daily
  // 拿日历按天明细。usage-daily 的 data 本身就是按天铺开的扁平行，分桶即得每格数据，
  // 所以不需要按天逐日打 30 个请求（早期方案就是那样，既慢又容易触发平台限流：
  // 约每 3 个请求就 503 一次，而且 503 要挂满 5s 才返回）。
  ...
  const fetchUpstreamUsage = async (cookie) => {
    ...
    // 两个接口并发：panel 取汇总/拆分/Top 花费，usage-daily 取日历按天明细。
    ...
    // panel 是页签主体（含 Top 花费，daily 给不了），失败即整体失败；
    // daily 失败则日历那一块留空，其余照常展示。
    if (panelJson === null) {
...
<script src="${jsUrl}"></script></body></html>`
  ...
  const serve = async (req, res) => {
    try {
      ...
      // 「用量」页签：panel（汇总/按模型/按客户端/Top 花费）+ usage-daily（日历按天）。
      ...
      if (pathname === '/dsh-tokenrhythm-bill/model-check' && req.method === 'POST') {
        ...
        if (!provider) { writeJson(res, { ok: false, error: '没有可用提供商（检查 settings.yaml 的 llm-pi-ai.providers）' }, 404); return }
        const key = resolveKey(provider)
      ...
      const stepListKeys = async () => {
        const r = await stepRpc(stepLib.STEP_DEV_SERVICE, 'ListAccessKeys', {})
        ...
        return { ok: true, keys: n.keys.filter((k) => !k.isDeleted), total: n.total }
      }
      if (pathname === '/dsh-tokenrhythm-bill/stepfun/keys' && req.method === 'GET') {
        writeJson(res, await stepListKeys())
        return
      }
      ...
      if (pathname === '/dsh-tokenrhythm-bill/stepfun/key-create' && req.method === 'POST') {
        ...
        const r = await stepRpc(stepLib.STEP_DEV_SERVICE, 'CreateAccessKey', { name })
        if (!r.ok) { writeJson(res, { ok: false, code: r.code, error: r.error }, 502); return }
      ...
      if (pathname === '/dsh-tokenrhythm-bill/stepfun/key-delete' && req.method === 'POST') {
        ...
        if (keyId === '') { writeJson(res, { ok: false, error: '缺少 keyId' }, 400); return }
        const r = await stepRpc(stepLib.STEP_DEV_SERVICE, 'DeleteAccessKey', { keyID: keyId })
        if (!r.ok) { writeJson(res, { ok: false, code: r.code, error: r.error }, 502); return }
        writeJson(res, await stepListKeys())
        return
      }
