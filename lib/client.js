  ...
  factory: (require) => {
    ...
    function renderBalanceTab({ manifest, balance, loadBalance, goSettings, showCalls, setShowCalls, trendHoverIdx, setTrendHoverIdx, sessionAccount, sessionAccountName }) {
      ...
      if (d) {
        ...
        // 命中率口径：缓存命中 ÷（输入 + 缓存命中）——平台日志里 input_tokens 不含
        // 缓存读（两列独立），输出是生成侧无缓存，故不计入分母。
        const cacheHitRate = (day) => {
          const denom = (day.inputTokens || 0) + (day.cacheReadTokens || 0)
          if (denom <= 0) return '—'
        ...
        rows.push(React.createElement('div', { className: 'dsh-mb-kv-grid', key: 'kv' },
          [
            ...
            ['输入 / 输出', day ? fmtTokens(day.inputTokens) + ' / ' + fmtTokens(day.outputTokens) + ' tokens' : '—'],
            ['缓存命中 / 命中率', day ? fmtTokens(day.cacheReadTokens) + ' tokens · ' + cacheHitRate(day) : '—', '缓存命中率 = 缓存命中 ÷（输入 + 缓存命中），输出不计入'],
