...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 入口常驻的总余额不能挂在它上面 ----
    const store = { open: false, view: 'models', alert: null, balanceCny: null };
    ...
    function EntryButton(props) {
      ...
      const bCny = s.balanceCny
      const balText = bCny !== null && bCny !== undefined ? '¥' + fmtCny(bCny) : null
      return React.createElement('button', btnProps,
        ...
        balText ? React.createElement('span', { className: 'dsh-mb-entry-bal' }, balText) : null,
      )
    }
    ...
    function Panel() {
      ...
      const loadBalance = useCallback(() => {
        ...
        jsonGet(API + '/balance').then((r) => {
          if (r && r.ok) {
            ...
            setStore({ alert: alertOf(r), balanceCny: r.balanceCny !== null && r.balanceCny !== undefined ? r.balanceCny : null })
          } else {
      ...
      const saveCookie = async (value) => {
        ...
        setCookieInput('')
        setStore({ alert: null, balanceCny: null }) // 旧会话的余额/预警立即失效
      ...
      const addAccount = async () => {
        ...
        if (r.loggedIn) {
          ...
          const m = await jsonGet(API + '/manifest')
          if (m && m.ok) setManifest(m)
          setStore({ alert: null, balanceCny: null })
          loadBalance()
        } else {
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        const check = async () => {
          ...
          if (b && b.ok) setStore({ alert: alertOf(b), balanceCny: b.balanceCny !== null && b.balanceCny !== undefined ? b.balanceCny : null })
        }
        const start = () => {
          check()
    ...
    const PANEL_CSS = `
      ...
      .dsh-mb-entry-bal{margin-left:auto;flex:none;padding-left:8px;font-variant-numeric:tabular-nums;
        color:var(--mb-sub);font-size:13px;letter-spacing:.2px;white-space:nowrap}
