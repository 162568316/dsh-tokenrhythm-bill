...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const store = { open: false, view: 'models', alert: null };
    ...
    function EntryButton(props) {
      ...
      return React.createElement('button', btnProps,
        React.createElement('span', { className: 'dsh-mb-entry-icon' },
          EntryMark({ size: wide ? 16 : 18 }),
          s.alert ? React.createElement('span', { className: 'dsh-mb-dot' }) : null,
        ),
        // label 与原生 newSessionLabel 同策略：rail 态卸载，展开时重挂并播 wide-in 淡入。
        wide ? React.createElement('span', { className: 'dsh-mb-entry-label wide-in' }, ENTRY_LABEL) : null,
      )
    }
    ...
    function renderBalanceTab({ manifest, balance, loadBalance, goSettings, showCalls, setShowCalls, trendHoverIdx, setTrendHoverIdx, sessionAccount }) {
      ...
      if (d) {
        ...
        rows.push(React.createElement('div', { className: 'dsh-mb-acct-line' + (sessionAccount ? '' : ' none'), key: 'acct' },
          React.createElement('span', { className: 'dsh-mb-acct-dot' }),
          '数据账号：' + (sessionAccount || '未登录（Cookie 模式）'),
        ))
    ...
    function renderSettingsTab({ manifest, sessionConfigured, sessionAccount, cookieInput, setCookieInput, cookieBusy, cookieMsg, saveCookie, accounts, addAcc, setAddAcc, addPw, setAddPw, showAddPw, setShowAddPw, accBusy, accMsg, addAccount, removeAccount, loginStored, showAccPw, setShowAccPw, showBackup, setShowBackup }) {
    ...
    const PANEL_CSS = `
      ...
      /* 侧栏入口：几何对齐原生「设置」触发行（SettingsRoot .trigger）：透明行、无边框、
       * h42/r12、图标起点 18px、文字起点一致（图标 16px + gap 8px + 14px/400 文字）。
       * 槽位容器 overflow-x:hidden 会裁掉负 margin 出血，故不用 -2px 出血，改用
       * 左 6px 内边距补偿（12px 侧栏内边距 + 6px = 设置行的 10px 出血 + 8px 内边距）。 */
      .dsh-mb-entry{position:relative;box-sizing:border-box;display:flex;align-items:center;gap:8px;
        width:100%;min-width:0;height:42px;margin:4px 0 0;padding:0 8px 0 6px;
      ...
      .dsh-mb-entry-label{min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
      ...
      /* 分类筛选行滚动固定：sticky 钉在滚动容器顶部，负 margin 铺满左右内边距
       * 并垫面板实色底，卡片列表从其下方滚过不透出。缓存标签并入行内保持一行，
       * 放不下时横向滚动（隐藏滚动条）。 */
      .dsh-mb-cats{position:sticky;top:0;z-index:2;display:flex;flex-wrap:nowrap;align-items:center;gap:6px;
        margin:0 -14px;padding:4px 14px 6px;background:var(--mb-panelBg);overflow-x:auto;scrollbar-width:none}
      .dsh-mb-cats::-webkit-scrollbar{display:none}
      .dsh-mb-cache-tag{flex:none;font-size:11px;line-height:24px;padding:0 8px;border-radius:12px;
      ...
      @media (prefers-reduced-motion:reduce){.dsh-mb-entry,.dsh-mb-entry-label.wide-in,.dsh-mb-trend-tip{transition:none;animation:none}
