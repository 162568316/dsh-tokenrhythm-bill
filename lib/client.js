  ...
  factory: (require) => {
    ...
    function Panel() {
      ...
      const createStepKey = async () => {
        ...
        setStepKeyBusy(true)
        setStepKeyMsg(null)
        console.log('[stepfun/key-create] payload=', JSON.stringify({ name }))
        const payload = { name }
        const r = await jsonPost(API + '/stepfun/key-create', payload)
        console.log('[stepfun/key-create] response=', JSON.stringify(r))
        alert('[stepfun/key-create]\n请求=' + JSON.stringify(payload) + '\n响应=' + JSON.stringify(r))
