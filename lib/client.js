...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // usage/panel 的 topCostCalls 时间：requestAt 是 UTC ISO，平台另给 time（UTC+8
    // HH:MM:SS）。优先用 time 去掉秒，省得再算一次时区；取不到再退回 requestAt。
    const fmtWhen = (requestAt, time) => {
      const t = /^\d{1,2}:\d{2}:\d{2}$/.test(String(time || '')) ? String(time).slice(0, 5) : null
      if (t !== null) return t
      const d = requestAt ? new Date(requestAt) : null
    ...
    // ---- 用量页签（0.5.5）：近 30 天汇总 + 30 天日历 + 按模型/客户端拆分 + Top 花费 ----
    // 注意：本函数与 Panel 组件体是兄弟作用域，拿不到里面的 useState，所以悬停态
    // calHover/setCalHover 必须由调用方当 props 传进来（与 renderBalanceTab 收
    // trendHoverIdx/setTrendHoverIdx 同一套路）。
    //
    // 数据来源（host 2 个上游请求）：
    ...
    function renderUsageTab({ usage, loadUsage, sessionAccount, calHover, setCalHover }) {
      ...
      rows.push(React.createElement('div', { className: 'dsh-mb-hero', key: 'hero' },
        ...
        React.createElement('div', { className: 'dsh-mb-kv-grid' },
          ...
          React.createElement('div', { className: 'dsh-mb-kv', key: 'k5' },
            React.createElement('span', { className: 'dsh-mb-kv-k' }, '输入'),
            React.createElement('span', { className: 'dsh-mb-kv-v' }, fmtTok(s.inputTokens))),
          React.createElement('div', { className: 'dsh-mb-kv', key: 'k6' },
            React.createElement('span', { className: 'dsh-mb-kv-k' }, '输出'),
            React.createElement('span', { className: 'dsh-mb-kv-v' }, fmtTok(s.outputTokens))),
        ),
      ))
      ...
      if (days.length > 0) {
        ...
        const noDataReason = (k) => (earliest !== '' && k < earliest
          ? '平台未返回该日明细'
          : '当天无调用')
        ...
        days.forEach((b, i) => cells.push({ b, key: b.date, idx: i }))
        rows.push(React.createElement('div', { className: 'dsh-mb-cal-wrap', key: 'cal' },
          ...
          // 覆盖度直说：空格子不代表页面坏了，是不在那几天没有调用。
          React.createElement('div', { className: 'dsh-mb-hint' },
            '其中 ' + activeDays + ' 天有调用，' + (days.length - activeDays) + ' 天无调用'
            + (earliest !== '' ? '；平台明细最早到 ' + earliest : '')),
          React.createElement('div', { className: 'dsh-mb-cal-hd' },
            USAGE_WD.map((w) => React.createElement('span', { key: w, className: 'dsh-mb-cal-wd' }, w))),
          React.createElement('div', {
            ...
            cells.map((c) => {
              ...
              const cls = 'dsh-mb-cal-cell' + (isToday ? ' today' : '') + (b.hasData ? '' : ' nodata')
              const tipRows = b.hasData
                ? (b.models.length > 0 ? b.models : [{ model: '当天无调用' }])
                : [{ model: noDataReason(b.date) }]
              return React.createElement('div', {
                className: cls,
                key: c.key,
                onMouseEnter: () => setCalHover(b.date),
              },
                React.createElement('span', { className: 'dsh-mb-cal-day' }, Number(p[2])),
                React.createElement('span', { className: 'dsh-mb-cal-cost' }, b.hasData ? '¥' + trimNum(b.costCny) : '—'),
                b.hasData ? React.createElement('i', {
                  className: 'dsh-mb-cal-bar',
                  style: { transform: 'scaleX(' + heat + ')' },
                }) : null,
                calHover === b.date ? React.createElement('div', {
                  className: 'dsh-mb-trend-tip' + (c.idx === 0 ? ' edge-l' : c.idx === days.length - 1 ? ' edge-r' : ''),
                },
                  React.createElement('div', { className: 'dsh-mb-trend-tip-head' },
                    React.createElement('span', null, b.date),
                    React.createElement('span', null, b.hasData
                      ? '¥' + trimNum(b.costCny) + ' · ' + b.calls.toLocaleString() + ' 次'
                      : '无数据')),
      ...
      if (byModel.length > 0) {
        const mx = Math.max.apply(null, byModel.map((g) => g.costCny).concat([0.0001]))
        rows.push(React.createElement('div', { className: 'dsh-mb-trend-wrap', key: 'bymodel' },
          React.createElement('div', { className: 'dsh-mb-day-title' }, '按模型（前 6，共 ' + (d.byModel || []).length + '）'),
          byModel.map((g, i) => React.createElement('div', { className: 'dsh-mb-rank', key: i },
            React.createElement('span', { className: 'dsh-mb-rank-k' }, g.model || g.modelId),
      ...
      if (byApp.length > 0) {
        rows.push(React.createElement('div', { className: 'dsh-mb-trend-wrap', key: 'byapp' },
          React.createElement('div', { className: 'dsh-mb-day-title' }, '按客户端（前 6，共 ' + (d.byClientApp || []).length + '）'),
          byApp.map((g, i) => React.createElement('div', { className: 'dsh-mb-rank', key: i },
            React.createElement('span', { className: 'dsh-mb-rank-k' }, g.clientApp || 'unknown'),
            React.createElement('span', { className: 'dsh-mb-rank-bar' },
              React.createElement('i', { style: { width: Math.max(3, Math.round(g.costCny / Math.max.apply(null, byApp.map((x) => x.costCny).concat([0.0001])) * 100)) + '%' } })),
            React.createElement('span', { className: 'dsh-mb-rank-v' }, '¥' + trimNum(g.costCny)),
            React.createElement('span', { className: 'dsh-mb-rank-c' }, g.calls + ' 次'),
          )),
        ))
      ...
      // ---- Top 花费调用（单笔最贵，usage-daily 给不了，只能来自 panel）----
      const tops = Array.isArray(d.topCostCalls) ? d.topCostCalls : []
      rows.push(React.createElement('div', { className: 'dsh-mb-day-title', key: 'toptitle', style: { marginTop: 4 } },
        '单笔花费最高（前 ' + Math.max(3, tops.length) + '）'))
      if (tops.length === 0) {
        rows.push(React.createElement('div', { className: 'dsh-mb-hint', key: 'topempty' }, '暂无单笔花费数据'))
      } else {
        rows.push(React.createElement('div', { className: 'dsh-mb-calls', key: 'tops' },
          tops.map((c, i) => React.createElement('div', { className: 'dsh-mb-call card', key: i, title: c.model || '' },
            React.createElement('div', { className: 'dsh-mb-call-top' },
              React.createElement('span', { className: 'dsh-mb-call-model' }, c.model || '未知模型'),
              React.createElement('span', { className: 'dsh-mb-call-cost' }, '¥' + trimNum(c.costCny)),
            ),
            React.createElement('div', { className: 'dsh-mb-call-meta' },
              fmtWhen(c.requestAt, c.time) + ' · ' + (c.clientApp || 'unknown')
              + ' · 入 ' + fmtTok(c.inputTokens) + ' / 出 ' + fmtTok(c.outputTokens)
              + (c.cacheReadTokens > 0 ? ' · 缓存 ' + fmtTok(c.cacheReadTokens) : '')
              + (c.reasoningTokens > 0 ? ' · 推理 ' + fmtTok(c.reasoningTokens) : '')),
            c.conversationSummary
              ? React.createElement('div', { className: 'dsh-mb-call-sum' }, c.conversationSummary)
              : null,
          )),
        ))
      }
    ...
    const PANEL_CSS = `
      ...
      /* ---- 用量页签：30 天日历 + 排行 ---- */
      .dsh-mb-cal-wrap{display:flex;flex-direction:column;gap:5px;padding:8px 10px;border-radius:10px;
        background:var(--mb-soft);border:1px solid var(--mb-lineSoft)}
      .dsh-mb-cal-hd{display:grid;grid-template-columns:repeat(7,1fr);gap:4px}
      .dsh-mb-cal-wd{font-size:10px;color:var(--mb-sub);text-align:center;font-variant-numeric:tabular-nums}
      .dsh-mb-cal{display:grid;grid-template-columns:repeat(7,1fr);gap:4px;position:relative}
      /* 一格一天：日期 + 当日金额 + 底部热度条（花费越高越宽）*/
      .dsh-mb-cal-cell{position:relative;display:flex;flex-direction:column;align-items:center;gap:1px;
        min-width:0;padding:4px 2px 5px;border-radius:7px;cursor:default;
        background:color-mix(in srgb,var(--mb-sub) 8%,transparent);border:1px solid transparent}
      .dsh-mb-cal-cell.blank{background:none;cursor:default}
      .dsh-mb-cal-day{font-size:10px;color:var(--mb-sub);font-variant-numeric:tabular-nums;line-height:13px}
      .dsh-mb-cal-cost{font-size:9.5px;font-weight:600;color:var(--mb-txt);font-variant-numeric:tabular-nums;
        max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .dsh-mb-cal-bar{display:block;width:100%;height:3px;border-radius:2px;transform-origin:left center;
        background:var(--mb-acc);opacity:.75;transition:transform var(--mb-fast) var(--mb-ease)}
      .dsh-mb-cal-cell:hover{background:color-mix(in srgb,var(--mb-sub) 18%,transparent)}
      .dsh-mb-cal-cell.nodata{opacity:.45}
      .dsh-mb-cal-cell.nodata .dsh-mb-cal-cost{font-weight:400}
      /* 今天：accent 描边，与趋势条的 today 同口径 */
      .dsh-mb-cal-cell.today{border-color:var(--mb-acc)}
      ...
      /* Top 花费调用：卡片式（不是余额页那种 dot/time 行），模型名 + 金额一行，
       * 时间/客户端/ token 一行摘要，最后是对话摘要。
       * .card 覆盖 .dsh-mb-call 的 flex-row（那是余额页「最近调用」的横向行布局）。*/
      .dsh-mb-call.card{flex-direction:column;align-items:stretch;gap:2px}
      .dsh-mb-call-top{display:flex;align-items:baseline;justify-content:space-between;gap:8px}
      .dsh-mb-call-meta{font-size:10.5px;color:var(--mb-sub);font-variant-numeric:tabular-nums;
        line-height:16px;word-break:break-all}
      .dsh-mb-call-sum{font-size:10.5px;color:var(--mb-sub);line-height:16px;
