...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Panel() {
      ...
      useEffect(() => {
        if (!s.open || !s.settingsOpen || settingsTab !== 'step') return
        loadStepKeys()
      }, [s.open, s.settingsOpen, settingsTab, loadStepKeys])
      const createStepKey = async () => {
        setStepKeyBusy(true)
        setStepKeyMsg(null)
        const r = await jsonPost(API + '/stepfun/key-create', { name: stepKeyName.trim() })
    ...
    function renderSettingsTab({ manifest, sessionConfigured, sessionAccount, cookieInput, setCookieInput, cookieBusy, cookieMsg, saveCookie, accounts, addAcc, setAddAcc, addPw, setAddPw, showAddPw, setShowAddPw, accBusy, accMsg, addAccount, removeAccount, loginStored, showAccPw, setShowAccPw, showBackup, setShowBackup, updInfo, updBusy, loadUpdate, copyUpdateCmd, updCopied, ignoreUpdate, entryMode, setEntryBalance, zcEnabled, setZcodeEnabled, settingsTab, setSettingsTab, stepCfg, stepAcc, setStepAcc, stepAccPw, setStepAccPw, showStepPw, setShowStepPw, stepAccBusy, stepAccMsg, addStepAccount, removeStepAccount, useStepAccount, setStepEntryPref, stepKeys, stepKeyName, setStepKeyName, stepKeyBusy, stepKeyMsg, stepKeyCreated, stepKeyCopied, stepKeyConfirm, createStepKey, copyStepCreatedKey, copyStepKey, deleteStepKey }) {
      ...
      return React.createElement('div', { className: 'dsh-mb-section' },
        ...
        // ---- 阶跃接口密钥（控制台「接口密钥」页同数据）：新增 / 复制 / 删除。
        // 完整值只在创建响应出现一次（平台策略），列表只回掩码；复制时 host 现拉
        // 列表取回完整值写剪贴板，界面绝不明文渲染。删除两段式确认防误触。----
        stepCfg && stepCfg.configured ? React.createElement('div', { className: 'dsh-mb-set-card', key: 'stepkeys' },
          React.createElement('div', { className: 'dsh-mb-set-title' }, '接口密钥（API Key）'),
          React.createElement('div', { className: 'dsh-mb-set-desc' },
            '与阶跃控制台「接口密钥」页同数据，最多 10 把。密钥泄露或不再使用时直接在这里删除（立即失效）。'),
          // 新建入口
          React.createElement('div', { className: 'dsh-mb-key-create' },
            React.createElement('input', {
              className: 'dsh-mb-input', style: { minHeight: 0, padding: '6px 10px' },
              placeholder: '新密钥名称（不超过 20 字符）',
              value: stepKeyName,
          ...
          // 创建成功：完整密钥只显示一次
          stepKeyCreated ? React.createElement('div', { className: 'dsh-mb-created', key: 'created' },
          ...
          // 列表
          stepKeys === null || stepKeys.loading
            ? React.createElement('div', { className: 'dsh-mb-skel-rows', key: 'ld', role: 'status', 'aria-label': '加载中' },
                [38, 62, 50].map((w, i) => React.createElement('div', { className: 'dsh-mb-skel', key: i, style: { width: w + '%' } })),
              )
            : stepKeys.error
              ? React.createElement('div', { className: 'dsh-mb-notice err', key: 'err' }, '加载失败：' + stepKeys.error)
              : (stepKeys.list || []).length === 0
                ? React.createElement('div', { className: 'dsh-mb-notice', key: 'empty' }, '还没有密钥，填个名称创建一个')
                : React.createElement('div', { className: 'dsh-mb-keys-list', key: 'ls' },
          ...
          stepKeyMsg ? React.createElement('div', { className: 'dsh-mb-cookie-msg' + (stepKeyMsg.ok ? '' : ' err') }, stepKeyMsg.text) : null,
          React.createElement('div', { className: 'dsh-mb-hint' },
