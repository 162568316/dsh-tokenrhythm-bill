 ...
 *                 click a card to copy its model id; the connectivity dot is
 *                 auto-lit from the status-page probe (no manual requests);
 *       状态    — status.moonlink.top service health via host /status: overall
 *                 banner, compact per-channel grid (tap for 24h/7d/90d detail),
 *                 recent incidents;
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 入口常驻的总余额不能挂在它上面 ----
    const store = { open: false, view: 'models', alert: null, balanceCny: null, expiringItems: [], entryBalMode: 'total', provider: 'tr', stepEntry: null, stepConfigured: true, stepTakeover: true };
    ...
    function EntryButton(props) {
      ...
      const s = useStore()
      // 阶跃胶囊接管标记（面板切到阶跃且有可显示数据时成立）；悬浮卡（基元语义）让位
      ...
      const title = stepOn
        ...
        : s.alert && !stepProv
          ...
          : stepProv ? STEP_ENTRY_LABEL : ENTRY_LABEL
      ...
      const hovCapable = hovItems.length > 0 && !s.open && !stepOn
      ...
      const btnProps = {
        ...
        'aria-label': stepProv ? STEP_ENTRY_LABEL : ENTRY_LABEL,
        ref: btnRef,
        ...
        React.createElement('button', btnProps,
          wide ? React.createElement('span', { className: 'dsh-mb-entry-left' },
            React.createElement('span', { className: 'dsh-mb-entry-icon' },
              stepProv ? StepMark({ size: 16 }) : EntryMark({ size: 16 }),
              s.alert && !stepProv ? React.createElement('span', { className: 'dsh-mb-dot' }) : null,
            ),
            React.createElement('span', { className: 'dsh-mb-entry-label wide-in' }, stepProv ? STEP_ENTRY_LABEL : ENTRY_LABEL),
          ) : React.createElement('span', { className: 'dsh-mb-entry-icon' },
            stepProv ? StepMark({ size: 18 }) : EntryMark({ size: 18 }),
            s.alert && !stepProv ? React.createElement('span', { className: 'dsh-mb-dot' }) : null,
          ),
          wide && balText ? React.createElement('span', { className: 'dsh-mb-entry-bal' + (s.alert && !stepProv ? ' alert' : '') }, balText) : null,
        ),
    ...
    function Panel() {
      const s = useStore()
      const [view, setView] = useState('balance') // 'balance' | 'models' | 'status' | 'settings'；默认落地余额（页签第一项）
      ...
      // 服务状态页签：数据 / 选中的通道详情（芯片点击展开）。选中态挂在 Panel
      // 层——renderStatusTab 是纯渲染函数不能带 hook（与 catFilter 同一套约定）。
      const [statusData, setStatusData] = useState(null)
      const [statusRange, setStatusRange] = useState('24h') // 视图区间（官方默认 24 小时）
      const [statusOpen, setStatusOpen] = useState({}) // 厂商分组折叠覆盖；未设置时=有问题自动展开
      // ---- 阶跃（StepFun）面板：提供商切换 + 用量/账户两页签 + 设置卡状态 ----
      const [provider, setProvider] = useState('tr') // 'tr' | 'step'（manifest.step.prefs.provider 播种）
      ...
      // 设置是右上角齿轮切入的临时视图：进出时记住/恢复上一个页签（两家各记各的）。
      ...
      const toggleSettings = useCallback(() => {
        setView((v) => {
          const back = provider === 'step' ? lastStepTabRef.current : lastTabRef.current
          const next = v === 'settings' ? back : 'settings'
          setStore({ view: next })
          return next
        })
      }, [provider])

      // ---- 阶跃：面板提供商切换（标题分段器）；选择持久化到 host step.prefs（跨会话记忆）。
      // 切到阶跃 → 胶囊跟随（store.provider/stepEntry 由 EntryButton 消费）。----
      ...
      const switchProvider = useCallback((p) => {
        const want = p === 'step' ? 'step' : 'tr'
        if (want === provider) return
        setProvider(want)
        setStore({ provider: want })
        ...
        setView((v) => {
          ...
          return String(v).indexOf('step-') === 0 ? lastTabRef.current : v
        })
      }, [provider])
      ...
      // provider 与 view 必须同族（TR 页签配基元、step 页签配阶跃）——否则持久化的
      // 阶跃 provider 在首包接管时，面板会以「阶跃页签 + 基元余额内容」的错位状态初始化
      ...
      useEffect(() => {
        if (!s.open || provider !== 'step' || view === 'settings') return
        loadStepPlan()
        ...
        jsonGet(API + '/manifest').then((r) => {
          ...
          if (r && r.ok && r.step) {
            ...
            if (!providerInitRef.current) { // 首包播种：上次选中的提供商（未配置阶跃一律落基元）
              ...
              const want = r.step.configured && r.step.prefs && r.step.prefs.provider === 'step' ? 'step' : 'tr'
              setProvider(want)
              setStore({ provider: want })
            }
          }
      ...
      // 服务状态：「状态」页签打开时 60s 轮询。失败时保留上次成功数据，
      // 只在没有任何数据时才落错误态。
      const statusRangeRef = useRef('24h')
      const statusSnapRef = useRef(0) // 上次快照 checkedAt：变化时重置分组折叠（对齐官方每次刷新重算）
      const loadStatus = useCallback((range) => {
        const r = typeof range === 'string' && range !== '' ? range : statusRangeRef.current
        statusRangeRef.current = r
        jsonGet(API + '/status?range=' + r).then((res) => {
          if (res && res.ok) {
            setStatusData(res)
            if (res.checkedAt && res.checkedAt !== statusSnapRef.current) {
              statusSnapRef.current = res.checkedAt
              setStatusOpen({}) // 新快照 → 折叠按「问题组自动展开」重新计算（与官方页一致）
            }
            return
          }
          setStatusData((cur) => (cur && cur.ok ? cur : { ok: false, error: (res && res.error) || '无法连接插件后台' }))
        })
      }, [])
      useEffect(() => {
        if (!s.open || view !== 'status') return
        loadStatus()
        const timer = setInterval(loadStatus, 60 * 1000)
        return () => clearInterval(timer)
      }, [s.open, view, loadStatus])
      ...
      const stepReady = !!(stepCfg && stepCfg.configured) // 配好阶跃账号才出现标题切换器（未配置=零打扰）
      const effProv = provider === 'step' && stepReady ? 'step' : 'tr'
      return React.createElement('div', {
        ...
        React.createElement('div', { className: 'dsh-mb-head', onPointerDown: onHeaderDown },
          React.createElement('div', { className: 'dsh-mb-head-left' },
            React.createElement('span', { className: 'dsh-mb-head-title' }, effProv === 'step' ? STEP_ENTRY_LABEL : ENTRY_LABEL),
            stepReady
              ? React.createElement('div', { className: 'dsh-mb-prov' },
                React.createElement('button', {
                  className: 'dsh-mb-prov-btn' + (effProv === 'tr' ? ' active' : ''),
                  title: '基元律动面板', onClick: () => switchProvider('tr'),
                }, '基元'),
                React.createElement('button', {
                  className: 'dsh-mb-prov-btn' + (effProv === 'step' ? ' active' : ''),
                  title: '阶跃 Step Plan 面板', onClick: () => switchProvider('step'),
                }, '阶跃'),
              )
              : null,
          ),
        ...
        // 页签（设置经由右上角齿轮进入，不占页签）；两家各一套。
        React.createElement('div', { className: 'dsh-mb-tabs' },
          (effProv === 'step'
            ? [['step-usage', '用量'], ['step-account', '账户']]
        ...
        React.createElement('div', { className: 'dsh-mb-body' },
          ...
          view === 'status' ? renderStatusTab({ statusData, loadStatus, statusRange, setStatusRange, statusOpen, setStatusOpen }) : null,
    ...
    // 状态页签（status.moonlink.top 聚合）：整体状态文案照抄官方页 OVERALL_TEXT；
    // 厂商分组照抄官方页 VENDOR_MAP + vendorOf（前缀匹配，未命中归「其他」）。
    const ST_STATUS = { up: '在线', degraded: '降级', down: '故障' }
    const ST_TONE = {
      operational: ['所有系统运行正常', 'ok'],
      degraded_performance: ['部分服务性能下降', 'warn'],
      partial_outage: ['部分服务中断', 'err'],
      major_outage: ['重大服务中断', 'err'],
    }
    const ST_VENDORS = [
    ...
    const stVendorOf = (id) => {
      if (id === '@api') return '平台网关'
      for (const v of ST_VENDORS) {
        if (v.prefix && id.startsWith(v.prefix)) return v.name
      }
      return '其他'
    }
    ...
    // ---- 状态页签（status.moonlink.top 90d 聚合，host 2min 缓存 + stale 兜底）----
    // 纯渲染函数：选中详情状态挂在 Panel 层传入（同 catFilter 约定），此处无 hook。
    function renderStatusTab({ statusData, loadStatus, statusRange, setStatusRange, statusOpen, setStatusOpen }) {
      if (statusData === null) {
      ...
      if (!statusData.ok) {
        return React.createElement('div', { className: 'dsh-mb-section' },
          React.createElement('div', { className: 'dsh-mb-notice err' },
            '服务状态加载失败：' + (statusData.error || '未知错误'),
            React.createElement('div', { style: { marginTop: '6px' } },
              React.createElement('button', { className: 'dsh-mb-link', onClick: loadStatus }, '重试'))))
      }
      const d = statusData
      const models = Array.isArray(d.models) ? d.models : []
      const rows = []
      if (d.stale) {
        rows.push(React.createElement('div', { className: 'dsh-mb-notice', key: 'stale' },
          '上游暂时不可达，显示 ' + fmtCheckedAt(d.checkedAt * 1000) + ' 的缓存快照'))
      }
      // ---- 总览横幅 ----
      const toneInfo = ST_TONE[d.overall] || [d.overall, 'warn']
      const badList = models.filter((m) => m.id !== '@api' && m.status !== 'up')
      rows.push(React.createElement('div', { className: 'dsh-mb-st-banner', key: 'banner' },
        React.createElement('span', { className: 'dsh-mb-st-bigdot ' + toneInfo[1] }),
        React.createElement('div', { className: 'dsh-mb-st-banner-main' },
          React.createElement('div', { className: 'dsh-mb-st-title' }, toneInfo[0]),
          React.createElement('div', { className: 'dsh-mb-st-desc' },
            badList.length > 0
              ? badList.map((m) => m.id + '（' + (ST_STATUS[m.status] || m.status) + '）').join('、')
                + ' · 在线 ' + d.upCount + '/' + d.modelCount
              : '全部 ' + d.modelCount + ' 个通道在线（含网关入口）· 探测于 ' + fmtCheckedAt(d.checkedAt * 1000))),
        React.createElement('div', { className: 'dsh-mb-st-stats' },
          React.createElement('div', { className: 'dsh-mb-st-stat' },
            React.createElement('span', { className: 'dsh-mb-st-k' }, '今日可用性'),
            React.createElement('span', {
              className: 'dsh-mb-st-v' + (d.todayAvail === null || d.todayAvail === undefined ? ''
                : d.todayAvail >= 99 ? ' ok' : d.todayAvail >= 95 ? ' warn' : ' err'),
            }, d.todayAvail === null || d.todayAvail === undefined ? '—' : trimNum(d.todayAvail) + '%')),
          React.createElement('div', { className: 'dsh-mb-st-stat' },
            React.createElement('span', { className: 'dsh-mb-st-k' }, '平均首字延迟'),
            React.createElement('span', { className: 'dsh-mb-st-v' }, fmtMsShort(d.avgTtfb))),
          React.createElement('div', { className: 'dsh-mb-st-stat' },
            React.createElement('span', { className: 'dsh-mb-st-k' }, '未解决事故'),
            React.createElement('span', { className: 'dsh-mb-st-v' + (d.unresolvedCount ? ' err' : ' ok') }, String(d.unresolvedCount || 0))),
        ),
      ))
      // ---- 视图切换（官方 range-switch）：24小时 / 7天 / 90天，默认 24h。----
      const RANGE_LABELS = [['24h', '24小时'], ['7d', '7天'], ['90d', '90天']]
      rows.push(React.createElement('div', { className: 'dsh-mb-st-rangebar', key: 'rangebar' },
        React.createElement('span', { className: 'dsh-mb-st-k' }, '组件状态'),
        React.createElement('div', { className: 'dsh-mb-st-rswitch' },
          RANGE_LABELS.map((rl) => React.createElement('button', {
            key: rl[0],
            className: 'dsh-mb-st-rbtn' + (statusRange === rl[0] ? ' active' : ''),
            onClick: () => { if (statusRange !== rl[0]) { setStatusRange(rl[0]); loadStatus(rl[0]) } },
          }, rl[1])))))
      // ---- 厂商分组（官方页方式）：平台网关排最前，其余按 VENDOR_MAP 顺序，
      // 空组丢弃；组体照官方页 mrow：状态点 + 模型名 + 元信息（ctx · 价格）+
      // 首字延迟 + 可用率%，下方逐日柱条（90d）或细格子（24h/7d，官方同款）。----
      const latOf = (m) => (m.status === 'down' ? (m.http ? 'HTTP ' + m.http : '超时') : fmtMsShort(m.ttfb))
      const toneOf = (m) => (m.status === 'up' ? 'ok' : m.status === 'degraded' ? 'warn' : 'err')
      const dayKeys = Array.isArray(d.days) ? d.days : []
      const metaOf = (id) => (d.meta && d.meta[id]) || ''
      const pctTxt = (v) => (v === null || v === undefined ? '–' : trimNum(v) + '%')
      const pctColor = (v) => (v === null || v === undefined ? 'var(--mb-tert)'
        : v >= 99.5 ? 'var(--mb-ok)' : v >= 95 ? 'var(--mb-warnTx)' : 'var(--mb-err)')
      const seriesOf = (id) => (d.modelDaily && Array.isArray(d.modelDaily[id])) ? d.modelDaily[id] : []
      const is90 = (d.range || '90d') === '90d'
      const maxCells = d.range === '24h' ? 288 : 2016
      // 细格子区间可用率（官方口径：有探测的格子才算，down 之外都算 ok）
      const slotPct = (codes) => {
        let t = 0
        let ok = 0
        for (const ch of codes) { if (ch !== '-') { t++; if (ch !== 'd') ok++ } }
        return t > 0 ? Math.round((ok / t) * 10000) / 100 : null
      }
      // grid 展平的时间戳（官方同款 Date.parse UTC），供细格子悬停显示时间
      const fmtSlotTs = (ts) => {
        const t = new Date((ts || 0) * 1000)
        return !ts || Number.isNaN(t.getTime())
          ? '' : (t.getMonth() + 1) + '-' + t.getDate() + ' ' + String(t.getHours()).padStart(2, '0') + ':' + String(t.getMinutes()).padStart(2, '0')
      }
      const flatTs = []
      if (!is90 && Array.isArray(d.slotGrid)) {
        for (const pair of d.slotGrid) {
          if (!Array.isArray(pair) || !Array.isArray(pair[1])) continue
          for (const hm of pair[1]) flatTs.push(Date.parse(pair[0] + 'T' + hm + ':00Z') / 1000)
        }
      }
      // 24h/7d 重采样桶数（面板窄幅，按桶画格不出滚动条；可用率% 仍按全部探测次数算）
      const SLOT_BUCKETS = { '24h': 72, '7d': 84 }
      // 桶 = 连续 size 个格子取最差状态（d > g > u > -），记录桶内探测数 + 首末时间
      const bucketize = (codes, tsOff) => {
        const n = SLOT_BUCKETS[d.range] || 72
        const size = Math.max(1, Math.ceil(codes.length / n))
        const rank = { '-': 0, u: 1, g: 2, d: 3 }
        const buckets = []
        for (let i = 0; i < codes.length; i += size) {
          let worst = '-'
          let count = 0
          for (let j = i; j < Math.min(i + size, codes.length); j++) {
            const ch = codes[j]
            count++
            if (rank[ch] > rank[worst]) worst = ch
          }
          const last = Math.min(i + size, codes.length) - 1
          buckets.push({ worst, count, ts0: flatTs[tsOff + i], ts1: flatTs[tsOff + last] })
        }
        return buckets
      }
      const groups = {}
      for (const m of models) {
        const vn = stVendorOf(m.id)
        if (!groups[vn]) groups[vn] = []
        groups[vn].push(m)
      }
      const groupOrder = ['平台网关'].concat(ST_VENDORS.map((v) => v.name))
      for (const vn of groupOrder) {
        const gRows = groups[vn]
        if (!gRows || gRows.length === 0) continue
        const downs = gRows.filter((m) => m.status === 'down').length
        const degs = gRows.filter((m) => m.status === 'degraded').length
        const isOpen = statusOpen[vn] !== undefined ? statusOpen[vn] : (downs > 0 || degs > 0)
        const sumTone = downs > 0 ? 'err' : degs > 0 ? 'warn' : 'ok'
        const sumText = downs > 0 ? downs + ' 个中断' : degs > 0 ? degs + ' 个性能下降' : '全部正常'
        rows.push(React.createElement('div', { className: 'dsh-mb-st-vgroup' + (isOpen ? ' open' : ''), key: 'vg' + vn },
          React.createElement('button', {
            className: 'dsh-mb-st-vhead',
            onClick: () => setStatusOpen((cur) => ({ ...cur, [vn]: !isOpen })),
          },
            React.createElement('span', { className: 'dsh-mb-st-chev' }, '▶'),
            React.createElement('span', { className: 'dsh-mb-st-vname' }, vn),
            React.createElement('span', { className: 'dsh-mb-st-vcnt' }, gRows.length + ' 个组件'),
            React.createElement('span', { className: 'dsh-mb-st-vstat' },
              React.createElement('span', { className: 'dsh-mb-st-dot ' + sumTone }),
              React.createElement('span', { className: 'dsh-mb-st-vstat-t ' + sumTone }, sumText))),
          isOpen ? React.createElement('div', { className: 'dsh-mb-st-vbody' },
            gRows.map((m) => {
              const full = (d.slotCodes && d.slotCodes[m.id]) || ''
              const codes = !is90 && full.length > maxCells ? full.slice(-maxCells) : full
              const tsOff = flatTs.length - codes.length
              const up = is90 ? m.a90 : slotPct(codes)
              const buckets = is90 ? null : bucketize(codes, tsOff)
              return React.createElement('div', { className: 'dsh-mb-st-mrow', key: m.id },
              React.createElement('div', { className: 'dsh-mb-st-mrow-top' },
                React.createElement('span', { className: 'dsh-mb-st-dot ' + toneOf(m) }),
                React.createElement('span', { className: 'dsh-mb-st-mname', title: m.id },
                  m.id === '@api' ? 'API 网关' : m.id),
                React.createElement('span', { className: 'dsh-mb-st-mmeta' },
                  m.id === '@api' ? 'OpenAI 兼容端点' : metaOf(m.id)),
                React.createElement('span', { className: 'dsh-mb-st-msp' }),
                React.createElement('span', { className: 'dsh-mb-st-mlat', title: '最近探测首字延迟' }, latOf(m)),
                React.createElement('span', { className: 'dsh-mb-st-mpct', style: { color: pctColor(up) }, title: is90 ? '90 天可用性' : (d.range === '24h' ? '24 小时可用性' : '7 天可用性') },
                  pctTxt(up))),
              React.createElement('div', { className: 'dsh-mb-st-cells' },
                is90
                  ? seriesOf(m.id).map((v, i) => React.createElement('div', {
                      key: i,
                      className: 'dsh-mb-st-cell'
                        + (v === null || v === undefined ? '' : v >= 995 ? ' up' : v >= 950 ? ' degraded' : ' down'),
                      title: (dayKeys[i] || '') + ' · 可用性 '
                        + (v === null || v === undefined ? '无数据' : (v / 10).toFixed(1) + '%'),
                    }))
                  : buckets.map((b, i) => React.createElement('div', {
                      key: i,
                      className: 'dsh-mb-st-cell'
                        + (b.worst === 'u' ? ' up' : b.worst === 'g' ? ' degraded' : b.worst === 'd' ? ' down' : ''),
                      title: m.id + ' · '
                        + (b.count > 0
                          ? fmtSlotTs(b.ts0) + (b.count > 1 ? '~' + fmtSlotTs(b.ts1) : '')
                          : '') + ' · '
                        + (b.worst === '-' ? '无数据' : ST_STATUS[b.worst === 'u' ? 'up' : b.worst === 'g' ? 'degraded' : 'down'])
                        + (b.count > 0 ? ' · ' + b.count + ' 次探测' : ''),
                    }))))
            })) : null))
      }
      // ---- 最近事故（官方状态页时间线风格：竖轨时间线 + 未解决红点脉冲 + 状态软徽章）----
      const incs = (Array.isArray(d.incidents) ? d.incidents : []).slice(0, 8)
      const openInc = incs.filter((it) => !it.resolvedAt).length
      rows.push(React.createElement('div', { className: 'dsh-mb-st-inccard', key: 'inc' },
        React.createElement('div', { className: 'dsh-mb-st-inc-hd' },
          React.createElement('span', { className: 'dsh-mb-st-k' }, '最近事故 · 自动检测'),
          React.createElement('span', { className: 'dsh-mb-st-inc-count' + (openInc > 0 ? ' err' : '') },
            openInc > 0 ? openInc + ' 条未解决' : '已全部恢复')),
        incs.length === 0
          ? React.createElement('div', { className: 'dsh-mb-st-chart-empty' }, '暂无事故记录 · 一切平稳')
          : incs.map((it, i) => React.createElement('div', { className: 'dsh-mb-st-inc' + (it.resolvedAt ? '' : ' open'), key: i },
              React.createElement('div', { className: 'dsh-mb-st-inc-rail' },
                React.createElement('span', { className: 'dsh-mb-st-inc-dot ' + (it.resolvedAt ? 'ok' : 'err') })),
              React.createElement('div', { className: 'dsh-mb-st-inc-main' },
                React.createElement('div', { className: 'dsh-mb-st-inc-title' }, it.title),
                it.detail ? React.createElement('div', { className: 'dsh-mb-st-inc-why' }, it.detail) : null,
                React.createElement('div', { className: 'dsh-mb-st-inc-when' },
                  React.createElement('span', null, fmtCheckedAt(it.startedAt * 1000) + ' 开始'),
                  React.createElement('span', { className: 'dsh-mb-st-inc-chip ' + (it.resolvedAt ? 'ok' : 'err') },
                    it.resolvedAt ? '已恢复 · 持续 ' + Math.max(1, Math.round((it.resolvedAt - it.startedAt) / 60)) + ' 分钟' : '未解决')))))))
      rows.push(React.createElement('div', { className: 'dsh-mb-hint', key: 'hint' },
        '数据来自 status.moonlink.top · host 缓存 2 分钟 · ' + (d.cached ? '本次为缓存结果' : '本次为实时抓取')))
      return React.createElement('div', { className: 'dsh-mb-section' }, rows)
    }
    ...
    function renderBalanceTab({ manifest, balance, loadBalance, goSettings, showCalls, setShowCalls, trendHoverIdx, setTrendHoverIdx, sessionAccount, sessionAccountName }) {
      ...
      if (d) {
        ...
        // 当日使用情况（本地 0 点起，聚合调用日志；含缓存命中与命中率）。
        ...
        // trendMeta.total 是平台真实总条数：分页被硬上限截断时（truncated），合计次数以
        // total 为准、金额注明「仅统计最新 N 条」——不再把截断值当全量装准。
        if (Array.isArray(d.trend) && d.trend.length > 0) {
          const meta = d.trendMeta && Number.isFinite(d.trendMeta.total) && d.trendMeta.total > 0 ? d.trendMeta : null
          const max = Math.max.apply(null, d.trend.map((b) => b.costCny).concat([0.01]))
          const totalCost = d.trend.reduce((acc, b) => acc + b.costCny, 0)
          const totalCalls = meta ? meta.total : d.trend.reduce((acc, b) => acc + b.calls, 0)
          ...
          rows.push(React.createElement('div', { className: 'dsh-mb-trend-wrap', key: 'trend' },
            React.createElement('div', { className: 'dsh-mb-day-title' },
              '近 7 天花费 · 合计 ¥' + (meta ? '≥' : '') + trimNum(totalCost) + ' · ' + totalCalls + ' 次'),
            meta && meta.truncated ? React.createElement('div', { className: 'dsh-mb-hint' },
              '近 7 天调用 ' + totalCalls + ' 次，金额仅统计最新 ' + meta.fetched + ' 条（分页上限），偏小仅作趋势参考') : null,
    ...
    const PANEL_CSS = `
      ...
      /* 状态页签（0.3.3）：总览横幅 / 矩阵表 / 走势图 / 事故时间线。 */
      .dsh-mb-st-banner{display:flex;align-items:center;gap:12px;padding:12px 14px;border-radius:12px;
        border:1px solid var(--mb-lineSoft);background:var(--mb-soft);flex-wrap:wrap}
      .dsh-mb-st-bigdot{flex:none;width:12px;height:12px;border-radius:50%;position:relative}
      .dsh-mb-st-bigdot.ok{background:var(--mb-ok)}
      .dsh-mb-st-bigdot.warn{background:var(--mb-warn)}
      .dsh-mb-st-bigdot.err{background:var(--mb-err)}
      .dsh-mb-st-bigdot::after{content:"";position:absolute;inset:-4px;border-radius:50%;background:inherit;
        opacity:.3;animation:dsh-mb-st-pulse 2.2s var(--mb-ease) infinite}
      @keyframes dsh-mb-st-pulse{0%{transform:scale(.55);opacity:.4}70%,100%{transform:scale(1.25);opacity:0}}
      .dsh-mb-st-banner-main{flex:1;min-width:150px}
      .dsh-mb-st-title{font-size:14px;font-weight:700}
      .dsh-mb-st-desc{font-size:11px;color:var(--mb-sub);margin-top:1px}
      .dsh-mb-st-stats{display:flex;gap:16px;flex-wrap:wrap}
      .dsh-mb-st-stat{display:flex;flex-direction:column;gap:1px}
      .dsh-mb-st-k{font-size:10px;color:var(--mb-tert)}
      .dsh-mb-st-v{font-size:15px;font-weight:700;font-variant-numeric:tabular-nums}
      .dsh-mb-st-v.ok{color:var(--mb-ok)}
      .dsh-mb-st-v.warn{color:var(--mb-warnTx)}
      .dsh-mb-st-v.err{color:var(--mb-err)}
      .dsh-mb-st-dot{flex:none;width:7px;height:7px;border-radius:50%}
      .dsh-mb-st-dot.ok{background:var(--mb-ok)}
      .dsh-mb-st-dot.warn{background:var(--mb-warn)}
      .dsh-mb-st-dot.err{background:var(--mb-err)}
      .dsh-mb-st-inccard{border:1px solid var(--mb-lineSoft);border-radius:10px;padding:10px 12px}
      .dsh-mb-st-chart-head{display:flex;align-items:center;gap:8px;margin-bottom:6px}
      .dsh-mb-st-chart-head .dsh-mb-st-k{font-weight:600}
      /* 厂商分组折叠面板：照官方页 —— 组头「组名 · N 通道 · 摘要」，问题组自动展开。 */
      .dsh-mb-st-vgroup{border:1px solid var(--mb-lineSoft);border-radius:10px;overflow:hidden}
      .dsh-mb-st-vgroup + .dsh-mb-st-vgroup{margin-top:6px}
      .dsh-mb-st-vhead{display:flex;align-items:center;gap:7px;width:100%;border:none;background:transparent;
        padding:8px 10px;cursor:pointer;font-size:12px;color:var(--mb-txt);text-align:left;
        transition:background-color var(--mb-fast) var(--mb-ease)}
      .dsh-mb-st-vhead:hover{background:var(--mb-soft)}
      .dsh-mb-st-chev{flex:none;font-size:9px;color:var(--mb-tert);transition:transform var(--mb-fast) var(--mb-ease)}
      .dsh-mb-st-vgroup.open .dsh-mb-st-chev{transform:rotate(90deg)}
      .dsh-mb-st-vname{font-weight:700}
      .dsh-mb-st-vcnt{color:var(--mb-tert);font-size:10.5px}
      .dsh-mb-st-vstat{margin-left:auto;display:inline-flex;align-items:center;gap:5px}
      .dsh-mb-st-vstat-t.ok{color:var(--mb-ok)}
      .dsh-mb-st-vstat-t.warn{color:var(--mb-warnTx)}
      .dsh-mb-st-vstat-t.err{color:var(--mb-err)}
      .dsh-mb-st-vbody{padding:2px 10px 10px}
      /* 通道行（官方 mrow）：状态点 + 模型名 + 元信息 + 首字延迟 + 90 天可用率%，
       * 下方 90 根逐日柱条（flex 均分；绿≥99.5 / 琥珀≥95 / 红；无数据灰）。 */
      .dsh-mb-st-mrow{padding:7px 0 8px;border-top:1px solid var(--mb-lineSoft)}
      .dsh-mb-st-mrow:first-child{border-top:none;padding-top:2px}
      .dsh-mb-st-mrow-top{display:flex;align-items:center;gap:7px;font-size:11.5px;min-width:0}
      .dsh-mb-st-mname{font-weight:600;max-width:45%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .dsh-mb-st-mmeta{color:var(--mb-tert);font-size:10px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .dsh-mb-st-msp{flex:1}
      .dsh-mb-st-mlat{flex:none;color:var(--mb-sub);font-size:10.5px;font-variant-numeric:tabular-nums}
      .dsh-mb-st-mpct{flex:none;font-weight:700;font-size:11px;font-variant-numeric:tabular-nums}
      .dsh-mb-st-cells{display:flex;gap:1px;margin-top:5px}
      .dsh-mb-st-cell{flex:1;height:18px;border-radius:1px;background:var(--mb-lineSoft)}
      .dsh-mb-st-cell.up{background:var(--mb-ok)}
      .dsh-mb-st-cell.degraded{background:var(--mb-warn)}
      .dsh-mb-st-cell.down{background:var(--mb-err)}
      /* 视图切换（官方 range-switch）+ 细格子滚动条（官方/DeepSeek 风格固定宽格子）。 */
      .dsh-mb-st-rangebar{display:flex;align-items:center;gap:10px;margin:10px 2px 8px}
      .dsh-mb-st-rswitch{display:inline-flex;gap:4px;margin-left:auto}
      .dsh-mb-st-rbtn{border:1px solid var(--mb-lineSoft);background:transparent;border-radius:7px;padding:2px 9px;
        font-size:10.5px;color:var(--mb-sub);cursor:pointer;
        transition:all var(--mb-fast) var(--mb-ease)}
      .dsh-mb-st-rbtn:hover{color:var(--mb-txt);background:var(--mb-soft)}
      .dsh-mb-st-rbtn.active{color:#fff;background:var(--mb-acc);border-color:var(--mb-acc)}
      .dsh-mb-st-chart-empty{padding:20px 0 14px;text-align:center;color:var(--mb-sub);font-size:12px}
      /* 事故时间线：竖轨连接各条目，未解决红点脉冲，已恢复绿点；悬停行浮起。 */
      .dsh-mb-st-inc-hd{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:2px}
      .dsh-mb-st-inc-count{flex:none;font-size:10px;color:var(--mb-sub);display:inline-flex;align-items:center;gap:5px}
      .dsh-mb-st-inc-count::before{content:"";width:6px;height:6px;border-radius:50%;background:var(--mb-ok)}
      .dsh-mb-st-inc-count.err{color:var(--mb-err);font-weight:600}
      .dsh-mb-st-inc-count.err::before{background:var(--mb-err);animation:dsh-mb-st-pulse 1.6s ease-out infinite}
      .dsh-mb-st-inc{display:flex;gap:10px;padding:8px 6px;border-radius:8px;transition:background var(--mb-fast) var(--mb-ease)}
      .dsh-mb-st-inc:hover{background:color-mix(in srgb,var(--mb-soft) 60%,transparent)}
      .dsh-mb-st-inc.open{background:color-mix(in srgb,var(--mb-err) 5%,transparent)}
      .dsh-mb-st-inc-rail{flex:none;width:10px;position:relative;display:flex;justify-content:center;align-items:flex-start;padding-top:5px}
      .dsh-mb-st-inc-rail::before{content:"";position:absolute;top:15px;bottom:-8px;left:50%;width:2px;transform:translateX(-50%);
        background:var(--mb-lineSoft);border-radius:1px}
      .dsh-mb-st-inc:last-child .dsh-mb-st-inc-rail::before{display:none}
      .dsh-mb-st-inc-dot{width:8px;height:8px;border-radius:50%;background:var(--mb-ok);position:relative;z-index:1;
        box-shadow:0 0 0 3px color-mix(in srgb,var(--mb-ok) 18%,transparent)}
      .dsh-mb-st-inc-dot.err{background:var(--mb-err);
        box-shadow:0 0 0 3px color-mix(in srgb,var(--mb-err) 20%,transparent)}
      .dsh-mb-st-inc-dot.err::after{content:"";position:absolute;inset:-3px;border-radius:50%;background:inherit;
        animation:dsh-mb-st-pulse 1.6s ease-out infinite}
      .dsh-mb-st-inc-main{flex:1;min-width:0}
      .dsh-mb-st-inc-title{font-size:12px;font-weight:600;line-height:1.35}
      .dsh-mb-st-inc-why{font-size:11px;color:var(--mb-sub);margin-top:2px;word-break:break-all;
        display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
      .dsh-mb-st-inc-when{font-size:10px;color:var(--mb-tert);margin-top:4px;display:flex;gap:8px;align-items:center;flex-wrap:wrap}
      .dsh-mb-st-inc-chip{flex:none;font-size:10px;font-weight:600;line-height:14px;border-radius:5px;padding:1px 7px}
      .dsh-mb-st-inc-chip.ok{color:var(--mb-ok);background:color-mix(in srgb,var(--mb-ok) 13%,transparent)}
      .dsh-mb-st-inc-chip.err{color:var(--mb-err);background:color-mix(in srgb,var(--mb-err) 13%,transparent)}
      @media (prefers-reduced-motion:reduce){.dsh-mb-entry,.dsh-mb-entry-label.wide-in,.dsh-mb-trend-tip,.dsh-mb-hov{transition:none;animation:none}
        .dsh-mb-st-bigdot::after,.dsh-mb-st-inc-dot.err::after,.dsh-mb-st-inc-count.err::before{animation:none}
        .dsh-mb-st-chev{transition:none}}
`;
