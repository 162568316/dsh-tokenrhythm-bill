...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Panel() {
      ...
      // 桌面通知开关（prefs.notify，默认开）：host 按它决定是否弹系统通知。
      ...
      useEffect(() => {
        ...
        jsonGet(API + '/prefs').then((r) => {
          ...
          setNotifyOn(r.prefs.notify !== false) // 默认开；显式 false 才关
        })
      ...
      // 桌面通知开关：本地立即生效 + 持久化到 prefs（host 轮询按它决定是否弹系统通知）。
      const setNotifyEnabled = useCallback((on) => {
        setNotifyOn(!!on)
        void jsonPost(API + '/prefs', { prefs: { notify: !!on } })
      }, [])
    ...
    function renderSettingsTab({ manifest, sessionConfigured, sessionAccount, cookieInput, setCookieInput, cookieBusy, cookieMsg, saveCookie, accounts, addAcc, setAddAcc, addPw, setAddPw, showAddPw, setShowAddPw, accBusy, accMsg, addAccount, removeAccount, loginStored, showAccPw, setShowAccPw, showBackup, setShowBackup, updInfo, updBusy, loadUpdate, copyUpdateCmd, updCopied, ignoreUpdate, entryMode, setEntryBalance, notifyOn, setNotifyEnabled }) {
      ...
      return React.createElement('div', { className: 'dsh-mb-section' },
        ...
        // 桌面通知：低余额 / 限时到期弹系统通知（DSH 原生 desktopRuntime.notifyAttention
        // 通道，由 host 在余额轮询后判定并去重）。仅桌面壳生效；窗口聚焦时壳自动不弹。
        React.createElement('div', { className: 'dsh-mb-set-card' },
          React.createElement('div', { className: 'dsh-mb-set-title' }, '桌面通知'),
          React.createElement('div', { className: 'dsh-mb-seg-line' },
            React.createElement('span', { className: 'dsh-mb-seg-desc' }, '低余额 / 限时到期时弹系统通知；仅桌面版生效，窗口聚焦时不打扰'),
            React.createElement('span', { className: 'dsh-mb-seg-row' },
              React.createElement('button', {
                className: 'dsh-mb-cat' + (notifyOn ? ' active' : ''),
