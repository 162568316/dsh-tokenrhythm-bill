 ...
 *
 * 2026-09-24 实测补记（真发一次登录测得，非推断）：
 *   - cookie 通道 Oasis-Token 长度 ~656，JSON 体 accessToken.raw 长度 ~344，**两者不是同一个 token**，
 *     但 JWT exp 与 accessToken.duration 三者完全一致：**都是 1800 秒 = 30 分钟**。
 *     → 30 分钟这个寿命是平台事实，不是保守估计；也确实没有"更长的那个 token"可捡。
 *   - refresh_token 有效期 30 天（JWT exp = 720 小时），**但它不是续期凭据**——
 *     真正的换发端点是 stepRefreshUrl()（见下），凭据走 Cookie 而非请求体
 *     （响应里那个 refreshToken 是轮换产物，服务端自己维护）。
 *
...
/**
 * proto bool 在 connect JSON 里序列化成 true/false（**不是** 1/0）；
 * 个别网关回 0/1 或 "1"/"true" 字符串，三种形态都认——否则默认徽标永不亮、
 * 已删除密钥（isDeleted:true）会过滤不掉而重新出现在列表里。
 */
function jbool(v) {
...
/**
 * 控制台「接口密钥」相关（2026-09 反解 platform.stepfun.com 前端 bundle 坐实）：
 * 服务就是 step.openapi.devcenter.Dashboard（与套餐查询同服务），方法：
 *   ListAccessKeys   Request{1 filter:bool,2 project_id}  Response{1 status,2 desc,3 accessKeys[],4 total}
 *   CreateAccessKey  Request{1 name,2 project_id}          Response{1 status,2 desc,3 keyInfo:AccessKey}
 *   DeleteAccessKey  Request{1 keyID}                      Response{1 status,2 desc}
 *   ModifyAccessKey  Request{1 keyID,2 name}               Response{1 status,2 desc}
 *   RefreshAccessKey Request{1 keyID}                      Response{1 status,2 desc}（轮换后重拉列表拿新值）
 * AccessKey{1 name,2 keyId:int64,3 accessKey,4 lastUsedAt:int64,5 createdAt:int64,
 *           6 isDefault,7 isDeleted,8 org_id,9 project_id,10 status}
 * int64 走 connect JSON 字符串通道；列表里 accessKey 是**完整值**（控制台前端也只在
 * 展示端掩码、复制端取全值）——host 掩码后才下发浏览器，完整值经 /stepfun/key 按需单取。
 */

/** 接口密钥上限（平台硬限「最多支持创建10个」，超限创建必失败，提前拦） */
const STEP_KEY_MAX = 10

/** 控制台同款掩码：前 6 + … + 后 6（短串退化为打点） */
function maskStepKey(s) {
  const v = typeof s === 'string' ? s : ''
  if (v === '') return ''
  if (v.length <= 6) return v
  if (v.length < 9) return v + '.'.repeat(v.length - 6)
  return v.slice(0, 6) + '...' + v.slice(-6)
}

/** int64 时间戳秒/毫秒双口径（>1e12 视为毫秒）→ ms；缺值/0 → null */
function stepTsToMs(v) {
  const x = jnum(v)
...
/**
 * ListAccessKeysResponse(JSON) → 密钥摘要列表（掩码下发，完整值留 host）。
 * isDeleted=1 的平台侧删除标记由调用方过滤；残缺字段全部降级，绝不抛出。
 */
function normalizeStepKeys(json) {
  const j = jobj(json)
  if (j === null) return { ok: false, keys: [], total: 0 }
  const keys = (Array.isArray(j.accessKeys) ? j.accessKeys : [])
    .map((r) => jobj(r))
    .filter((r) => r !== null)
    .map((r) => {
      const idNum = jnum(r.keyId)
      return {
        keyId: jstr(r.keyId) !== null ? jstr(r.keyId) : (idNum !== null ? String(idNum) : ''),
        name: jstr(r.name) || '',
        masked: maskStepKey(jstr(r.accessKey) || ''),
        isDefault: jbool(r.isDefault),
        isDeleted: jbool(r.isDeleted),
        createdAt: stepTsToMs(r.createdAt),
        lastUsedAt: stepTsToMs(r.lastUsedAt),
      }
    })
  return { ok: j.status === 1, keys, total: jnum(j.total) !== null ? jnum(j.total) : keys.length }
}
...
/**
 * 续期 URL（2026-09-24 实测反解）：
 * PassportService/RefreshToken，方法名按 gRPC-web JSON 通道的 PascalCase 约定试出
 * （RefreshAccessToken / RenewToken / RefreshUserToken / Refresh / RefreshSession / SilentRenew 全部 404）。
 * 请求体 `{}` 即可——凭据走 Cookie（Oasis-Token + Oasis-Webid），二者缺一不可
 * （只缺 Webid → 400 "oasis header is invalid"；只缺 Token → 400 "token is missing"）。
 * 实测语义：**过期但真实**的 token 可以换回本人有效会话（mode=2、oasis_id 一致、RPC 200）；
 * 而签名非法的垃圾 token 只会铸出设备匿名 token（mode=1、身份不同、RPC 401）——
 * 与标准 JWT 刷新一致：签名得是真的，只是过期了。
 * 响应同时轮换 refreshToken（每次都给新的），Set-Cookie 里的 Oasis-Token 才是平台长令牌。
 */
function stepRefreshUrl() {
  return STEP_ACCOUNT_BASE + '/passport/proto.api.passport.v1.PassportService/RefreshToken'
}
...
export {
  ...
  parseAuthResponse, parseRegisterDeviceResponse, jwtClaims, jwtExpiry,
  // 归一
  normalizeStepPlanStatus, normalizeStepCredit, normalizeStepUsages, normalizeStepAccounts, normalizeStepWallet,
  normalizeStepKeys, maskStepKey,
  // 胶囊
  pickStepEntryDisplay,
  // 常量
  STEP_ACCOUNT_BASE, STEP_PLATFORM_BASE, STEP_OASIS_HEADERS, STEP_DEV_SERVICE, STEP_KEY_MAX,
  stepRegisterUrl, stepSignInUrl, stepRpcUrl, stepRefreshUrl,
}
