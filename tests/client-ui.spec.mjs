...
test('阶跃：host 路由与登录通道（cookie-only 实测规则焊死在源码）', () => {
  ...
  assert.ok(hostSrc.includes('truncated: !complete()') && hostSrc.includes('TREND_MAX_PAGES = 60') && hostSrc.includes('TREND_WINDOW_DAYS = 90'), '趋势必须「最近 7 个有记录日」口径 + 自适应早停 + 硬上限——旧版 15 页×100 静默截断，重用户旧日子显示幽灵 ¥0（实测 3538 次只统计到 1500）')
  assert.ok(src.includes('个有记录日') && src.includes('仅统计最新'), '趋势截断提示缺失——未能凑齐完整 7 天时须注明仅统计最新 N 条')
